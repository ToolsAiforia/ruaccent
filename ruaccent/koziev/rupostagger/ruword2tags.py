...
class RuWord2Tags:
    ...
    def load(self, dict_path=None, workdir: str | None = None):
        if workdir is not None:
            self.db_filepath = os.path.join(workdir, "database", "ruword2tags.db")
        else:
        ...
        with open(os.path.join(workdir if workdir else module_folder ,"ruword2tags.dat"), 'rb') as f:
            data = pickle.load(f)
